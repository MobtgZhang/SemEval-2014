...
class Similarity(nn.Module):
    ...
    def forward(self,lvec,rvec):
        ...
        out = torch.sigmoid(self.wh(vec_dist.sum(dim=1)))
        out = F.softmax(self.wp(out),dim=1)
        return out
class RNNSentiment(nn.Module):
    def __init__(self,vocab_size,embedding_dim,mem_dim,hid_dim,num_layers,rnn_type,dropout,bidirectional,num_classes):
        ...
        self.similarity = Similarity(hidden_size,hid_dim,num_classes)
